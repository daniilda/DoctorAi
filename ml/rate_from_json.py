...
def get_summary(input_example: dict) -> dict:
    ...
    for doctor_id in unique_ids:
        ...
        for i in range(len(desease)):
            if test_doctor['desease'] == desease['Диагноз'].iloc[i] and desease['Обязательность'].iloc[i] == 'Обязательно':
                y_true_nesses[i] = 1
        ...
        for i in range(len(desease)):
            if test_doctor['desease'] == desease['Диагноз'].iloc[i] and desease['Обязательность'].iloc[i] == 'По показанию':
                y_true_unnesses[i] = 1
        
        #заполнение y_pred_unnesses
        for i in range(len(desease)):
            for k in range(len(unnesses)):
                if test_doctor['desease'] == desease['Диагноз'].iloc[i] and unnesses[k] == desease['Направление'].iloc[i]:
                    #print(desease['Диагноз'].iloc[i], desease['Направление'].iloc[i])
                    y_pred_nesses[i] = 1
        ...
        for i in range(len(desease)):
            if ((y_pred_nesses[i] == 1 and y_true_nesses[i] == 1) or (y_pred_unnesses[i] == 1 and y_true_unnesses[i] == 1)):
